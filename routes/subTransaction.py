...
from schemas.transaction_schemas import SubTransaction, SubTransactionCreate, SubTransactionUpdate

router = APIRouter()

@router.post("/subtransaction/post", response_model=SubTransaction, tags=["SubTransaction"])
def create_subtransaction(subtransaction: SubTransactionCreate, db: Session = Depends(get_db)):
    return crud.create_subtransaction(db=db, subtransaction=subtransaction)

@router.get("/subtransactions/get", response_model=List[SubTransaction], tags=["SubTransaction"])
...
@router.get("/subtransaction/get/{subtransaction_id}", response_model=SubTransaction, tags=["SubTransaction"])
...
@router.put("/subtransaction/put/{subtransaction_id}", response_model=SubTransaction, tags=["SubTransaction"])
def update_subtransaction(subtransaction_id: int, subtransaction_update: SubTransactionUpdate, db: Session = Depends(get_db)):
