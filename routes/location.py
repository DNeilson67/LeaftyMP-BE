...
@router.get('/location/get', response_model=List[schemas.Location], tags=["Location"])
def get_location(limit: int = 100, db: Session = Depends(get_db)):
    return crud.get_location(db=db, limit=limit)

@router.get('/location/getuserid/{user_id}', response_model=schemas.Location, tags=["Location"])
...
def patch_location_by_user_id(location: schemas.LocationPatch, user_id:str, db: Session=Depends(get_db)):
