...
@router.post("/flour/post", response_model=Flour, tags=["Flour"])
def create_flour(flour: FlourCreate, db: Session = Depends(get_db)):
...
@router.get("/flour/get/{flour_id}", response_model=Flour, tags=["Flour"])
...
@router.put("/flour/put/{flour_id}", response_model=Flour, tags=["Flour"])
def update_flour(flour_id: int, flour: FlourUpdate, db: Session = Depends(get_db)):
    update_flour = crud.update_flour(db=db, flour_id=flour_id, flour_update=flour)
    if not update_flour:
        raise HTTPException(status_code=404, detail="flour not found")
    return update_flour

@router.put("/flour/update_status/{flour_id}", response_model=Flour, tags=["Flour"])
def update_flour_status(flour_id: int, status_update: FlourStatusUpdate, db: Session = Depends(get_db)):
