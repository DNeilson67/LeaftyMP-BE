...
@router.post("/product/post", response_model=schemas.Products, tags=["Products"])
def create_product(product: schemas.ProductsCreate, db: Session = Depends(get_db)):
    return crud.create_product(db=db, product=product)

@router.get("/products/get", response_model=List[schemas.Products], tags=["Products"])
def get_products(skip: int = 0, limit: int = 10, db: Session = Depends(get_db)):
    return crud.get_products(db=db, skip=skip, limit=limit)

@router.get("/product/get/{product_id}", response_model=schemas.Products, tags=["Products"])
...
@router.put("/product/put/{product_id}", response_model=schemas.Products, tags=["Products"])
